...
namespace ExpenseTracker.Services.Implementations
{
    public class ExpenseService : IExpenseService
    {
        ...
        public void SetExpenses(IEnumerable<ExpenseVm> expenses)
        {
            try
            {
                var expenseObjects = expenses.Select(x =>
                    new Expense
                    {
                        ValueOfExpense = x.ValueOfExpense,
                        ReasonForExpense = x.ReasonForExpense,
                        Created = DateTime.ParseExact(x.Created, "yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture)
                    }).ToList();

                _baseRepository.CreateMany<Expense>(expenseObjects);
            }
            catch (Exception e)
            {
                _logger.LogError(e.Message + "=>" + e.InnerException);
